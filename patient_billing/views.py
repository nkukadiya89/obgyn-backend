...
@validate_permission("patient_billing", "delete")
def delete(request):
    data = {}
    ...
    try:
        patient_billing = PatientBillingModel.objects.filter(
            patient_billing_id__in=del_id["id"]
        )
    except PatientBillingModel.DoesNotExist:
        data["success"] = False
        data["msg"] = "Record does not exist"
...
def patch(request, id):
    ...
    if request.method == "POST":
        serializer = PatientBillingSerializers(
            patient_billing, request.data, partial=True
        )
...
@api_view(["GET"])
@authentication_classes([JWTAuthentication])
@validate_permission_id("patient_billing", "view")
...
def get(request, id=None):
    ...
    try:
        ...
        data["total_record"] = len(patient_billing)
        patient_billing, data = filtering_query(
            patient_billing, query_string, "patient_billing_id", "PATIENTBILLING"
        )
