...
class PatientPrescriptionAPI(APIView):
    ...
    def put(self, request, id):
        data = {}
        try:
            patient_prescription = PatientPrescriptionModel.objects.filter(pk=id).first()
        ...
        if request.method == "PUT":
            serializer = PatientPrescriptionSerializers(patient_prescription, request.data)
...
def patch(request, id):
    ...
    if request.method == "POST":
        serializer = PatientPrescriptionSerializers(patient_prescription, request.data, partial=True)
...
def get(request, id=None):
    ...
    try:
        if id:
            patient_prescription = PatientPrescriptionModel.objects.filter(pk=id, deleted=0)
        else:
            patient_prescription = PatientPrescriptionModel.objects.filter(deleted=0)

        data["total_record"] = len(patient_prescription)
        patient_prescription, data = filtering_query(patient_prescription, query_string, "patient_prescription_id",
                                                     "PATIENTPRESCRIPTION")
