...
def usg_form_report_rpt(request, usg_form_id, language_id=None):
    ...
    context["hospital_address"] = "".join(
        [
            " ",
            patient_opd.consulted_by.hospital.city.city_name if patient_opd.consulted_by.hospital.city.city_name else " ",
            " ",
            patient_opd.consulted_by.hospital.district.district_name if patient_opd.consulted_by.hospital.district.district_name else " ",
            " ",
            patient_opd.consulted_by.hospital.taluka.taluka_name if patient_opd.consulted_by.hospital.taluka.taluka_name else " ",
            " ",
            patient_opd.consulted_by.hospital.state.state_name if patient_opd.consulted_by.hospital.state.state_name else " ",
        ]
    )
    ...
    m_cnt=f_cnt=0
    usg_child_list = USGFormChildModel.objects.filter(patient_usgform=usg_form,deleted=0).order_by("child_dob")
    print(usg_child_list)
    for usg_child in usg_child_list:
        ...
        if usg_child.child_gender == "MALE":
            m_cnt = m_cnt + 1
            usgchild["male"] = usgchild["male"] + "(" + str(m_cnt) + ") - "+ str(age_year) + " y/s ,"
        elif usg_child.child_gender == "FEMALE":
            f_cnt = f_cnt + 1
            usgchild["female"] = usgchild["female"] + "(" + str(f_cnt) + ") - "+ str(age_year) 
