...
def delete(request):
    ...
    try:
        template_header = TemplateHeaderModel.objects.filter(template_header_id__in=del_id["id"])
    except TemplateHeaderModel.DoesNotExist:
        data["success"] = False
        data["msg"] = "Record does not exist"
        data["data"] = []
...
def patch(request, id):
    ...
    if request.method == "POST":
        serializer = TemplateHeaderSerializers(template_header, request.data, partial=True)
...
def get(request, id=None):
    ...
    try:
        ...
        template_header, data = filtering_query(template_header, query_string, "template_header_id", "TEMPLATEHEADER")
